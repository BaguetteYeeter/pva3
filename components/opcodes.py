...
def reg(bios: bios.BIOS) -> None:
    address = bios.ram.fetch(bios.cpu.Registers.CurrentInstruction, 2)
    bios.cpu.Registers.CurrentInstruction += 2
    address = ord(address[0]) + (ord(address[1]) * 0x100)
    a = bios.ram.fetch(address, 2)
    fh, sh = ord(a[1]), ord(a[0])
    address = sh + (fh * 0x100)
    bios.cpu.Registers.A = ord(bios.ram.fetch(address, 1))

def call(bios: bios.BIOS) -> None:
    devid = ord(bios.ram.fetch(bios.cpu.Registers.CurrentInstruction, 1))
    bios.cpu.Registers.CurrentInstruction += 1
    address = bios.ram.fetch(bios.cpu.Registers.CurrentInstruction, 2)
    bios.cpu.Registers.CurrentInstruction += 2
    address = ord(address[0]) + (ord(address[1]) * 0x100)
    bios.call(devid, address)

codes = {0x00: nop, 0x01: add, 0x02: sub, 0x03: mul, 0x04: div, 0x05: mod, 0x0e: dec, 0x0f: inc, 0x10: jmp, 0x11: je, 0x12: jne, 0x13: jg, 0x14: jge, 0x15: jl, 0x16: jle, 0x41: call, 0xa1: lda, 0xa2: ldb, 0xa3: ldc, 0xa4: ldd, 0xb1: sta, 0xb2: stb, 0xb3: stc, 0xb4: std, 0xc1: sea, 0xc2: seb, 0xc3: sec, 0xc4: sed, 0xaa: reg}